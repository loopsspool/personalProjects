...
def bulba_scrape_pokemon(poke_num, allow_download=False):
    bulba_scrape_config = generate_config_dict(BULBA_FILE_STARTER_URL, get_bulba_img, allow_download)

    # Game Sprites
    scrape_imgs(poke_num, "obtainable_game_filenames", bulba_game_sprite_translate, exclusions=bulba_doesnt_have_images_for, has_animation=True, save_path=save_directories["Game Sprites"]["path"], config_dict=bulba_scrape_config)
    # Drawn Imgs
    scrape_imgs(poke_num, "drawn_filenames", drawn_translate, exclusions=None, has_animation=False, save_path=save_directories["Drawn"]["path"], config_dict=bulba_scrape_config)
    # Home Sprites
